...
public:
    ...
    Character* getWinner() {
        if (!p1->isAlive()) return p2;
        if (!p2->isAlive()) return p1;
        return nullptr;
    }
};
...
Character* createCharacter(int choice) {
    ...
    if (choice == 2) return new Mage("Mage");
    if (choice == 3) return new Archer("Archer");
    if (choice == 4) return new Monster("Monster");
    return nullptr;
}
